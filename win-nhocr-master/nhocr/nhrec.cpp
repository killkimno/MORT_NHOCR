...
int NHrec :: rec_addstr(wchar_t *line, const wchar_t *str, int bufsize){
   
	if ( wcslen(line) + wcslen(str) >= (size_t)bufsize )  return(-1);
    wcscat(line,str);
...
int NHrec :: rec_gramd_solve(wchar_t *buffer, size_t size) {
    if (!gramd_enabled)
        return 0;
#ifdef HAVE_LIBGRAMDCLIENT      // NOT EDITTED FOR wchar_t
...
int NHrec :: rec_character(SIPImage *image, CharBox *cb, \
    ...
    if ( resultTable != 0 && Rec.resultTable != 0 ){
...
int NHrec :: rec_line(SIPImage *image, wchar_t *resultline, int bufsize){
    int	cid;
    CharBox	*cba = 0;
    CharBox	*cba_raw;
    CharBox	*cb, *cb0;
    ...
    resultline[0] = '\0';
		...
		for (i = 0; cba[i].nbox != 0; i += cba[i].nbox ){
			...
			if (!isTest)
			{
				...
				if (cb0->alphamode == 0 || cb->alphamode == 0){
					...
					if ((int)avrcwidth * .8 < cb->xs - xe0){
						if (0 > rec_addstr(resultline, L" ", bufsize)){
							break;
						}
						else {
							// Added space
			...
			if (Rec.resultTable != NULL && !isTest)
			{
...
//	if ( cblist )  delete_cblist(cblist);
	if (cba != NULL)
    delete []cba;
	if (recResult_cList != NULL)
    delete []recResult_cList;
	if (recResult1List != NULL)
    delete []recResult1List;
	if (recResult2List != NULL)
