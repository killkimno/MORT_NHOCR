...
SIPImage	SIPImage0 = { 0,0,8,16,0,nullptr };


                /* Create image */

SIPImage *sip_CreateImage2(int width,int height,int depth, int tempBuffSize){
    SIPImage	*imgtmp;
    long		linesize;
    long		bufsize;
    void		**pdata;
    ...
    if ( nullptr == (imgtmp = (SIPImage *)malloc(sizeof(SIPImage))) ){
        return(nullptr);
    }
    if ( nullptr == (pdata = (void **)malloc(sizeof(void *) * height)) ){
        free((void *)imgtmp);
        return(nullptr);
    }
    imgtmp->pdata  = pdata;
    ...
    if ( nullptr == (imgtmp->data = (char *)malloc(bufsize)) ){
        free((void *)pdata);
        free((void *)imgtmp);
        return(nullptr);
    }
...
int sip_ClearLine(SIPImage *image,int y,int32 data){
    char	*p;
    if ( nullptr == image->data )  return(-1);
    if ( nullptr == (p = sip_getimgptr(image,y)) )  return(-1);
...
int sip_ClearImage(SIPImage *image,int32 data){
    ...
    if ( data )  db = (char)0xff;  else db = 0;
    if ( nullptr == image->data )  return(-1);
    if ( nullptr == (p = sip_getimgptr(image,0)) )  return(-1);
    for ( y=0 ; y < image->height ; y++ ){
...
int sip_CopyArea(SIPImage *src,SIPRectangle *srect,
    ...
    if ( nullptr == (sp = sip_getimgptr(src,srect->y)) )  return(-1);
    if ( nullptr == (dp = sip_getimgptr(dst,drect->y)) )  return(-1);
    if ( depth == 1 ){
