...
wchar_t	resultline[MaxLineLength + 10];
...
static int rec_multichar(SIPImage *image, NHrec *NHrec, \
    ...
    for ( n=0 ; n<nchar ; n++ ){
        ...
        for ( r=0 ; r<n_top ; r++ ){
            ...
            if ( cid < 0 )  break;
            wcsncpy(candchar, NHrec->cclist[cid].ccode, 14);
            if ( wcscmp(candchar,L"\\") == 0 ){
                wcscpy(candchar, L"\\\\");
            }
            if ( wcscmp(candchar,L" ") == 0 ){
                wcscpy(candchar, L"\\ ");
...
----------------------------------------------*/
static int LoadImage(int width, int height, uchar* data, bool isGrey = true)
{

	SIPImage	*imgbuf;


	if (0 == (imgbuf = sip_CreateImage(width, height, 8)))  return(-6);
...
void InitOcr()
{
	...
	api.setlibdir("F:\\download\\win-nhocr-master\\win-nhocr-master\\Debug\\Dic");
	api.setdiccodes("ascii+:jpn");
	api.setgramdportfile("PLM-jpn");
}
...
InitNHOCR()
{
	...
	if (sw_debug)  api.debug = 1;
	if (sw_alpha)  api.force_alpha = -1;
	if (sw_gramd)  api.gramd_enabled = 1;
...
extern "C" __declspec(dllexport)int ProcessNHocr(int imgWidth, int imgHeight, uchar* data, std::wstring* resultstring, int channel)
{
	...
	int width = imgWidth; int height = imgHeight;
	...
	*/
		...
		int tempResult = tempWidth * tempHeight;

		if (nullptr == (image = sip_CreateImage2(tempWidth, tempHeight, 8, tempResult)))
		{
			wcscat(resultline, L"Failed in text segmentation in the text block.");
			*resultstring = *resultstring + resultline;
			return(-6);
		}


		sip_ClearImage(image, 0);
		//image->data = new char[imgHeight*imgWidth];
		for (int i = 0; i < tempResult; i++)
		{
			image->data[i] = data[i * channel];
		}

		int nowHeight = 0;
		for (int i = 0; i < tempHeight; i++)
		{
			image->pdata[i] = &image->data[nowHeight];
			nowHeight = nowHeight + tempWidth;
		}
		...
		if (sw_mchar == 0){
			api.binarize_otsu(image, image, csize, sw_interpolate);
			api.auto_invert(image);
		}
		...
		if (sw_block){
			tlgroup.clearall();
			rect_pool.clear();
			if (0 != segment_lines(image, &rect_pool, &tlgroup, 0, 1, 1)){
				wcscat(resultline, L"Failed in text segmentation in the text block.");
				*resultstring = *resultstring + resultline;
		...
		if (sw_mchar){	// multi-character recognition mode
			if (rec_multichar(image, &api, sw_ntop, fp_out)){
				return(2);
			}
		}
		...
		/* ---- Recognize text line(s) ---- */
			...
			api.rec_line(image, resultline, MaxLineLength);

			wcscat(resultline, L"\n");
			if (WEOF == fputws(resultline, fp_out)){
				return(2);
			}
		}
			...
			for (int tlid = 0; tlid<tlgroup.groups; tlid++){
				TLimage = cutout_textline(image, &rect_pool, &tlgroup, tlid);

				if (TLimage == nullptr || TLimage == 0)
					break;
				api.rec_line(TLimage, resultline, MaxLineLength);
				wcscat(resultline, L"\n");
				*resultstring = *resultstring + resultline;
				/*
				if (WEOF == fputws(resultline, fp_out)){
				...
				*/
				...
				if (nullptr != TLimage){
					if (nullptr != TLimage->data){
						free((void *)TLimage->pdata);
						free((void *)TLimage->data);
						TLimage->data = nullptr;
					}
					free((void *)TLimage);
				}
				//sip_DestroyImage(TLimage);
...
int main(int ac,char *av[]){
	...
	*av = new char[5];
	...
	av[1] = "-block";
	av[2] = "-o";
	av[3] = "F:\\Project\\visualStudio Projects\\win-nhocr-master\\Release\\a.txt";
	av[4] = "F:\\Project\\visualStudio Projects\\win-nhocr-master\\Release\\out2.pbm";
	...
	NHrec.setlibdir("F:\\download\\win-nhocr-master\\win-nhocr-master\\Debug\\Dic");
	NHrec.setdiccodes("ascii+:jpn");
	NHrec.setgramdportfile("PLM-jpn");
    ...
    if ( 0 != strcmp(outfile, "-") ){
        if ( NULL == (fp_out = fopen(outfile,"w, ccs=UTF-8")) ){
  ...
  for ( ; argn < ac ; argn++ ){
    ...
    if ( sw_mchar ){	// multi-character recognition mode
        ...
        for ( int tlid=0 ; tlid<tlgroup.groups ; tlid++ ){
            ...
            wcscat(resultline, L"\n");
			stringTest = stringTest + resultline;
            if ( WEOF == fputws(resultline,fp_out) ){
                fprintf(stderr,"Can't write on \"%s\".\n",outfile);
                return(2);
            }
		...
		sip_DestroyImage(bimage);
		sip_DestroyImage(image);
		std::cout << *stringTest.c_str();
    }
        if ( 0 > save_bin_image("out12.pbm",bimage) ){
            fprintf(stderr,"Can't write on \"%s\".\n","out2.pbm");
        }
        return(2);
